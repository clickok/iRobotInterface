 ...
 * Author: BB
 * 
 * A small program for moving the iRobot Create to a default starting position
 * inside its microworld after a run, so that we can run multiple robots
...
/* loadCliffThresholds()
 * Loads the cliff sensor thresholds from the file "cliffThresholds.dat" and
 * places them in a global variable for use by other parts of the program.
 */
void loadCliffThresholds() {
  FILE *fd;
