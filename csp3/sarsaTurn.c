...
-----
  ...
  of places. Here are the steps:
    ...
    1. Find the sensor id of the new sensor in the iRobot Create Open 
       ...
       (packet) id 9. The file createcodes.h has a list of symbolic names
       many of the sensor id's, and the one you want to add may be there.
    ...
    2. While you have the Open Interface manual open to the page for
       ...
       produced by your sensor. How many bytes is it? How it the data
...
-----
  ...
  remote to stop your robot, before interrupting this program with ^C.
  ...
  This program was originally written by Rupam Mahmood (ashique@ualberta.ca)
  and Rich Sutton (rich@richsutton.com) in June, 2013.
 */
...
//sensory arrays:
#define M 1000
unsigned short  sCliffL[M], sCliffR[M], sCliffFL[M], sCliffFR[M], sAngle[M]; // small pos integers
...
void extractPacket()
{
	...
	sDeltaT[p] = (currentTime.tv_sec - lastPktTime.tv_sec)*1000 + ((double) currentTime.tv_usec - lastPktTime.tv_usec)/1000;
...
int main(int argc, char *argv[])
{
	...
	pthread_mutex_init(&pktNumMutex, NULL);
  	...
  	while (TRUE)
  	{
  		...
  		computeTime = (timeEnd.tv_sec-timeStart.tv_sec)*1000000 + (timeEnd.tv_usec-timeStart.tv_usec);
  		printf("Time for iteration (in microseconds): %ld\n", computeTime);
  		usleep(50000 - computeTime);
  		timeStart = timeEnd;
  		gettimeofday(&timeStart, NULL);
  		myPktNum = getPktNum();
