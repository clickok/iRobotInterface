...
#define INITIAL_RANDOM_VALUES 0
...
int epsilonGreedy(double Q[16][4], int s, int epsilon)
{
  int max, i;
  int myPktNum, p;
  int firstAction, localLastAction;

  myPktNum = getPktNum();
  p = (myPktNum - 1) % M;
  firstAction = sCliffFLB[p] || sCliffFRB[p];
  if (sCliffLB[p] || sCliffRB[p]) localLastAction = 2;
  else localLastAction = 3;
  if (rand()/((double)RAND_MAX+1) < epsilon) {
    printf("random action\n\n");
    return firstAction + rand()%(localLastAction + 1 - firstAction);
  } else {
    max = localLastAction;
    for (i = firstAction; i < localLastAction; i++)
      if (Q[s][i] > Q[s][max])
        max = i;
    return max;
  }
...
int main(int argc, char *argv[]) {
  ...
  double Visits[16][4];
  double stepsize = 0.1, lambda = 0.9, gamma = 0.9, epsilon = 0.01;
  int a, aprime;
  int s, sprime;
  int reward;
  int i, j;
  ...
  ubyte bytes[2];
  int rewardReport;
  struct timeval timeStart, timeEnd,timeBegin;
  ...
  char logfilename[50];
  sprintf(logfilename,"DistanceSARSA-y%d-m%02d-d%02d-h%02d-m%02d-s%02d.logfile",
	  (tB->tm_year+1900),(1+tB->tm_mon),tB->tm_mday,
	   tB->tm_hour,tB->tm_min,tB->tm_sec);
  logfile=fopen(logfilename,"w");
  if (logfile==NULL) {
    fprintf(stderr, "failed to open logfile for writing\n");
    return 0;
  }  
  

  char * ascdate=asctime(tB);
  fprintf(logfile,"# date= %s",ascdate);
  fprintf(logfile,"# commandlineargs= ");
  for (i=0;i<argc;i++) {
    fprintf(logfile,"%s " ,argv[i]);
  }
  fprintf(logfile,"\n");
  fprintf(logfile,"# stepsize= %f lambda= %f gamma= %f epsilon= %f InitialValue= %f \n",stepsize,lambda, gamma, epsilon, (double)INITIAL_VALUE);
  fprintf(logfile,"# Fields: TIMESTEP CLOCKTIME S A R S' A' \n");
  loadCliffThresholds();
  srand(0);
  ...
  for (i = 0; i < 16; i++)
    for (j = 0; j < 4; j++) {
      Q[i][j] = INITIAL_VALUE + INITIAL_RANDOM_VALUES*0.001*( rand()/((double) RAND_MAX) - 0.5);
      e[i][j] = 0;
      Visits[i][j]=0;
    }
  ...
  while (TRUE) {
    ...
    for (p = prevPktNum; p < myPktNum; p++) {
      ...
      printf("deltaT: %6.2f cliff sensors: %u(%u) %u(%u) %u(%u) %u(%u) distance: %hd\n",
	     sDeltaT[p%M],
    ...
    netrr+=reward*reward;
    printf("iteration: %d \n", iteration++ );
    double er=netr/iteration;
    printf("avg_r: %4.3f reward_sigma %4.3f\n", er, sqrt(netrr/iteration - er*er));
    printf("s a r s' a':%d %d %d %d %d\n", s, a, reward, sprime, aprime);
    for (i = 0; i < 16; i++) {
      printf("Action values (freqs) for state %2d: %6.2f (%5.3f) %6.2f (%5.3f) %6.2f (%5.3f) %6.2f (%5.3f)\n",i, Q[i][0], Visits[i][0]/iteration, Q[i][1], Visits[i][1]/iteration, Q[i][2],Visits[i][2]/iteration, Q[i][3],Visits[i][3]/iteration);
      //printf("Eligibility traces for state %d: %6.2f %6.2f %6.2f %6.2f\n", i, e[i][0], e[i][1], e[i][2], e[i][3]);
      for (j = 0; j < 4; j++) {
        Q[i][j] = Q[i][j] + stepsize*delta*e[i][j];
        e[i][j] = gamma*lambda*e[i][j];
      }
    }
    double netTime = ((timeStart.tv_sec-timeBegin.tv_sec)*1000000
		      + (timeStart.tv_usec-timeBegin.tv_usec))/1000000.;

    fprintf(logfile, "%d %9.3f %d %d %d %d %d\n",iteration,netTime,s,a,reward,sprime,aprime);
    fflush(logfile);
    s = sprime;
    a = aprime;

    setLastAction(aprime);
    prevPktNum = myPktNum;
  }
