/* sarsaLogs.c
 * Logging of packets to identify differences in behavior due to different
 * systems
 */
...
int main(int argc, char *argv[]) {
  ...
  ubyte bytes[2];
  int rewardReport;
  struct timeval timeStart, timeEnd, incrementBy, timeBegin;
  long computationTime;
  struct sigaction act;
  struct sigaction oldact;
	int iteration = 0;
  FILE *logfile;
  int rewardBuffer[1000];
  ...
  if (argc < 2) {
	...
	fprintf(logfile,"# Fields: TIMESTEP CLOCKTIME S A R S' A' pakNum r1 r2 r3 r4 r5 r6 r7 r8 r9 r10 r11 r12\n");
  ...
  for (i = 0; i < 16; i++)
    for (j = 0; j < 4; j++) {
      Q[i][j] = INITIAL_VALUE + 0.001*( rand()/((double) RAND_MAX) - 0.5);
      e[i][j] = 0;
    }
  ...
  while (TRUE) { // main agent loop
    ...
    for (pn = prevPktNum; pn < myPktNum; pn++) {
      p = pn % M;
      reward += sDistance[p];
	  rewardBuffer[rewardIndex++]=sDistance[p];
	...
	fprintf(logfile, "%d %9.3f %d %d %d %d %d ",iteration,netTime,s,a,reward,sprime,aprime);
	fprintf(logfile, "%d ",rewardIndex );

	  int xx=0;
	for(; xx<12; ++xx)
		fprintf(logfile, "%d ",rewardBuffer[xx]);
	fprintf(logfile, "\n");
